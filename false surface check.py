    ...
    def export_stl(self):
        ...
        def export_thread():
            try:
                ...
                # --- try to read spacing from DICOM and apply to verts if available ---
                if hasattr(self, "dicom_files") and self.dicom_files:
                    try:
                        ...
                        if ps is not None and st is not None:
                            ...
                            # skimage returns verts as (z, y, x) order (row-major). 
                            # We trust previous code's axis mapping — adapt as needed for your dataset.
                            verts[:, 2] *= spacing_xy[0]   # X
                            verts[:, 1] *= spacing_xy[1]   # Y
                            verts[:, 0] *= slice_thickness # Z
                            log_message("Using DICOM spacing for vertex coordinates.")
                    except Exception as e:
                        log_message(f"Warning: couldn't read DICOM spacing: {e}")
                ...
                pv_mesh = pv_mesh.fill_holes(1000.0)  # Adjust area as needed
                        ...
                        #voxelize  revise revise revise OMIT???
                try:
                   #voxel = pv.voxelize(pv_mesh,1.0)
                   #voxel_surface = voxel.extract_surface().triangulate().clean(tolerance=1e-3)
                   #log_message("voxel remeshing complete")
                    pass

                except Exception as e:
                    voxel_surface = pv_mesh
                    log_message(f"Voxel remeshing failed: {e}")

                    
                         #taubin smooth
                try:
                    voxel_surface = voxel_surface.smooth_taubin(n_iter=30, pass_band=.1)
                    log_message("Applied taubin smoothing to final mesh")
                except Exception as e:
                    log_message(f"taubin smoothing skipped:{e}")

                        #keep single component only
